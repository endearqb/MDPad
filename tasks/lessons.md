...
## 2026-04-22 Chart Editor Scope Control
- 当用户把图表编辑诉求收敛到“只改数据”时，不要继续保留图表类型、标题、图例、坐标轴这类展示控制；应优先把 modal 明确收缩成数据矩阵编辑器。
- 图表数据表若目标是接近 Excel，不要在单元格周围叠加额外卡片、颜色区和方向按钮；优先用连续网格、紧凑固定宽度和原生拖放表达排序。
- 系列颜色即使底层模型仍保留，也不要默认暴露为编辑能力；先按最小必要功能提供标签、系列名、数值和顺序编辑，再决定是否扩展。

## 2026-04-22 Chart Header Interaction Separation
- 图表表头里的“编辑、拖拽、删除”不能共用同一命中区；如果输入框覆盖了大部分头单元格，就不要再把 `draggable` 挂在整个 `th` 上。
- 对这类矩阵编辑器，优先采用“独立拖拽手柄 + 输入框 + 条件删除”的三段式结构：输入框只编辑，手柄只排序，删除默认隐藏到 hover/focus 再出现。
- 常驻删除按钮会让整张表一直处于高噪音状态；在不影响发现性的前提下，优先用 `focus-within` / `hover` 显隐，而不是始终高对比暴露。

## 2026-04-25 Fullscreen Chrome State
