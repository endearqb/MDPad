...
-   [v0.2.11 新增功能](#v0211-%E6%96%B0%E5%A2%9E%E5%8A%9F%E8%83%BD)
...
### Tiptap 3 官方 Markdown 链路

-   富文本编辑器已从 Tiptap 2 + `tiptap-markdown` 迁移到 Tiptap 3 + 官方 `@tiptap/markdown`。

-   Markdown 文档现在以 Markdown content type 进入编辑器，保存时使用 `editor.getMarkdown()` 输出，减少可视化编辑与源文件之间的格式漂移。

-   表格实现切换到官方 Tiptap 3 `TableKit`，替代旧的 MDPad 自定义表格 schema 与 resize 主链路。

### 更稳的富文本粘贴行为

-   剪贴板包含 `text/html` 时，交给 ProseMirror/Tiptap 原生粘贴链路处理，不再由旧 sanitizer 或 Markdown fallback 抢占。

-   只有剪贴板没有 HTML 且纯文本明显是 Markdown 时，才走官方 Markdown 插入。

-   图片文件粘贴仍保留附件库工作流，但已从通用富文本粘贴链路中隔离出来。

### 当前工作区能力

-   MDPad 继续支持打开和编辑 `.md`、`.markdown`、`.html`、`.htm`、`.py`、`.js`、`.ts`、`.json`。

-   Markdown 保留富文本/源码双视图，HTML 保留预览/源码双视图，代码文件直接进入 CodeMirror 6 源码编辑器。

-   受控 HTML 预览继续支持本地资源解析、沙箱脚本执行，以及外部链接交给系统浏览器打开。
...
-   基于 Tiptap 3 官方 Markdown manager 的富文本编辑链路。
