  ...
  return (
    <div className="app-modal-backdrop" onMouseDown={onCancel} role="presentation">
      <section
        ...
        role="dialog"
      >
        <header className="app-modal-header html-preview-svg-modal-header">
          <div>
          ...
          <button
            ...
            className="app-modal-btn is-ghost html-preview-svg-expand-btn"
            onClick={() => setIsExpanded((current) => !current)}
            title={
              isExpanded
                ? copy.htmlPreview.svgEditorRestoreWidth
                : copy.htmlPreview.svgEditorMaximizeWidth
            }
            type="button"
          >
            {isExpanded
                ...
                <div className="html-preview-svg-stage-overlay">
                  {history.present.items.map((item) => {
                    ...
                    const connectorHitPaddingX = isConnector
                      ? Math.max((18 / request.viewBox.width) * 100, 1.2)
                      : 0;
                    const connectorHitPaddingY = isConnector
                      ? Math.max((18 / request.viewBox.height) * 100, 1.2)
                      : 0;
                    ...
                    return (
                      <button
                        ...
                        aria-label={getDisplayLabel(copy, item)}
                        key={key}
                        onDoubleClick={(event) => {
                          event.preventDefault();
            ...
            {selectedItems.length > 1 && sharedStyle ? (
              ...
              <>
                ...
                {primarySelectedItem.canEditText ? (
                  <div className="html-preview-svg-text-field">
                    <textarea
                      aria-label={copy.htmlPreview.textLabel}
                      className="html-preview-textarea html-preview-svg-text-input"
                      onBlur={commitPendingFormEdit}
